...
resource "openvswitch_bridge" "ovsbr2" {
  name       = "ovsbr2"
...
resource "libvirt_network" "ovs_network" {
  depends_on = [openvswitch_bridge.ovsbr2]
...
resource "libvirt_cloudinit_disk" "commoninit" {
  count          = var.worker02_vm_count
  name           = "worker02-commoninit-${count.index}.iso"
  pool           = libvirt_pool.test_vm_storage_pool.name
...
data "template_file" "network_config" {
  ...
  vars = {
    ip_address = "192.168.102.${count.index + 10}"
...
resource "null_resource" "activate_actor" {
  ...
  provisioner "remote-exec" {
    connection {
      ...
      host     = "192.168.102.${count.index + 10}"
    }

    inline = [
      "/usr/local/go/bin/go env -w GOPROXY=https://goproxy.cn,direct",
      "cd /home/ubuntu/actor-platform/example && /usr/local/go/bin/go run worker/main.go worker --addr 192.168.102.${count.index + 10} --remote_addr 192.168.101.10 --remote_port 4968"
    ]
  }
}
